...
def _flat(tmp_path):
    """A settings object for the default flat layout: this file is about
    concurrent writers, not about where owners are filed."""
    return ContentSettings(data_dir=tmp_path / "data", db_path=tmp_path / "db.sqlite")


def test_concurrent_promotion_gives_every_writer_its_own_file(tmp_path):
    storage = JobStorage.from_settings(
        _flat(tmp_path), LOCAL_OWNER, "job_race"
    ).ensure()
...
def test_concurrent_copies_of_one_artifact_do_not_collide(tmp_path):
    ...
    storage = JobStorage.from_settings(
        _flat(tmp_path), LOCAL_OWNER, "job_copies"
    ).ensure()
...
def test_promotion_failure_does_not_reserve_the_name(tmp_path):
    ...
    storage = JobStorage.from_settings(
        _flat(tmp_path), LOCAL_OWNER, "job_fail"
    ).ensure()
