...
def delivery_root_for(settings, owner_id: str) -> Path | None:
    ...
    The one place that reads ``delivery_scope``. Callers ask for a root and
    get one; they never branch on the policy themselves, which is what keeps
    the rule from being re-implemented slightly differently in three places.

    ``per_owner`` validates the id as a path segment even though ids are
    generated by the engine and never by a client: an owner id becomes a
    directory name here, and belt-and-braces at the boundary costs nothing
    (INV-STORAGE-006).
    """
    scope = getattr(settings, "delivery_scope", "shared")
    if scope == "off":
        return None
    root = Path(settings.delivery_dir or (Path(settings.data_dir) / "delivery"))
    if scope == "per_owner":
        return root / safe_segment(owner_id, "owner_id")
    return root
