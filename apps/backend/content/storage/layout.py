...
from content.storage.roots import OwnerRoots, owner_roots
...
class JobStorage:
    ...
    def __init__(self, roots: OwnerRoots, job_id: str):
        """Paths for one job, under one owner's roots.

        The owner is carried by ``roots`` rather than by a bare id (ADR 0030):
        a caller cannot forget it, and cannot assemble a path of their own. It
        is a *filing* decision and never an authorization one — permission is
        decided in the database, against ``owner_id`` columns, before any path
        is built here. A path that happens to be reachable proves nothing.

        Where the owner level sits — nowhere in the flat layout, at
        ``users/<owner>`` in the per-user one — is the roots' business, not
        this class's (ADR 0037).
        """
        safe_segment(job_id, "job_id")
        self.owner_id = roots.owner_id
        self.job_id = job_id
        self.root = roots.job(job_id)
        self.sources = self.root / "sources"
        self.work = self.root / "work"
        self.artifacts = self.root / "artifacts"
        self.logs = self.root / "logs"
        self.snapshots = self.root / "snapshots"
        self.tmp = roots.job_tmp(job_id)

    @classmethod
    def from_settings(cls, settings, owner_id: str, job_id: str) -> "JobStorage":
        return cls(owner_roots(settings, owner_id), job_id)
...
def delivery_root_for(settings, owner_id: str) -> Path | None:
    """Where this owner's deliveries go, or ``None`` when delivery is off.

    Kept as the name every caller already uses; the answer now comes from the
    owner's roots, so the layout and the delivery scope are resolved in one
    place (ADR 0037) rather than half here and half there.
    """
    return owner_roots(settings, owner_id).output
