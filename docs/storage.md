...
| `CONTENT_DELIVERY_SCOPE` | derived from the auth mode | `shared` (one library), `per_owner` (one inside each owner's directory, `<data>/users/<owner>/output`) or `off` (no library; delivery refused with `delivery_not_supported`) |
