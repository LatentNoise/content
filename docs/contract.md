...
**`provider_args` (`url` source, advanced escape hatch)**: a list of raw arguments passed to the acquisition provider (yt-dlp), e.g. `["--proxy", "http://host:8080", "--limit-rate", "2M"]`. An **explicit, documented exception** to the "no provider leakage into the contract" principle, reserved for advanced users. Security guard rail: an **allowlist** — only network routing (`--proxy`, `--geo-verification-proxy`, `--source-address`, `--force-ipv4`/`-6`), geo bypass (`--geo-bypass`, `--geo-bypass-country`, `--xff`), pacing (`--limit-rate`, `--throttled-rate`, `--retries`, `--fragment-retries`, `--retry-sleep`, `--socket-timeout`, `--sleep-requests`, `--sleep-interval`/`--max-sleep-interval`, `--concurrent-fragments`, `--http-chunk-size`) and request identity (`--user-agent`, `--referer`) flags pass; **anything else is rejected** (`ValidationError`), including bare tokens that are neither a listed flag nor its value. An operator-side default also exists through `CONTENT_YTDLP_EXTRA_ARGS` (trusted, unfiltered), applied to every yt-dlp invocation.
...
| --- | --- | --- |
| `video` | `selection` (`max_height` a strict ceiling; `video_codec`/`audio_codec` in `prefer`\|`require` mode, D4; `audio_languages`: an **ordered** list of audio languages to include as tracks — >1 = embedded multi-audio, intersected with what the source offers, unavailable ones dropped with a `partial_output` warning), `container` (`source`\|`mkv`\|`mp4`), `processing` (`mode`: `auto`\|`copy`\|`remux`\|`transcode` — `transcode` valid but not implemented; `embed_metadata`, `embed_thumbnail`, `embed_chapters`, `embed_subtitles`: a list of languages embedded into the container, same rules; embedding ignored on file sources), `sponsorblock` (`remove`/`mark`: category lists; `cut_mode`: `keyframes`\|`precise`, default `keyframes`. `keyframes` is the fast, stream-preserving cut (INV-019): yt-dlp only *marks* the segments, and Content removes them itself with a keyframe-snapped stream-copy concat — codecs untouched, I/O speed, and the end of the file provably clean (yt-dlp's own stream-copy remover leaves a phantom keep-chunk on end-reaching segments that replays the final GOP with colliding timestamps — the historical stuttering tail). Honest costs: a boundary snaps to the nearest keyframe, and a mid-video seam may double ~2 reordered frames (≈66 ms). `precise` is the explicit opt-in to yt-dlp's `--force-keyframes-at-cuts`: exact boundaries, but the **whole file is re-encoded** at ffmpeg's default codecs — measured at 8 min 33 s of CPU against 17 s of download on a 2160p AV1 clip, returned as a 46% larger H.264/Vorbis file) | 1 |
...
| `pdf` | `page_size` (`a4`\|`letter`), `title` (empty = taken from the rendered material). Input: a readable source **or**, through `from_outputs`, any readable output (`summary`, `transcript`, `translation`, `chapters`, `markdown`, `document_text`). Asked plain on a source with no readable text, it derives the **summary** by default and the artifact says so in its name (ADR 0028); the `from_outputs` form always wins when declared. Rendered by an operator-selected implementation (Typst or ReportLab) — the renderer, its template and its fonts are **operator configuration, never contract**; the chosen backend appears in the artifact's provenance. See [operations/pdf-rendering.md](operations/pdf-rendering.md) | 1 |
...
| Method | Path | Role |
| --- | --- | --- |
| `GET` | `/config` | Client configuration: the ids of available credentials (never the paths/secrets), `surfaces` — the deployed UIs as `{kind, title, url}`, so a client can offer the others — `public_api_url`, where a browser reaches the engine and which every link a surface draws is built on, `retention.days` (0 = kept until deleted) and `quota_wall`, what a refused visitor may be offered (empty unless configured) |
...
| Method | Path | Role |
| --- | --- | --- |
| `POST` | `/auth/link` | Ask for a sign-in link. **Always 202 with the same body** — a different answer for a known address would be an account-enumeration oracle — carrying a fresh `reference`, the four-character code that ends the email's subject, so a person holding several links can tell the newest |
...
| `GET` | `/storage` | **What you hold**, broken down: `artifacts`, `delivery`, `uploads` (counted in `total_bytes`), plus `resources`, `history` and `tmp` (shown, not counted). No paths |
